...
def test_weather_service_successful_response():
    """
    WeatherService should return parsed JSON when the API responds normally.
    """
...
def test_weather_service_retries_then_fails():
    """
    WeatherService should retry multiple times and then raise WeatherApiError
    if all attempts fail (network issues, timeouts, etc.).
    """
    service = WeatherService(max_retries=2, retry_delay=0)  # faster tests
